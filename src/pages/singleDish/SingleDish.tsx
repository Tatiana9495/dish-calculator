...
const _SingleDish: React.FC<ISingleDish> = ({ dish }: ISingleDish): JSX.Element => {
    ...
    useFirestoreConnect([
      {
        ...
        orderBy: ['title', 'asc'],
        doc: dish.id,
        subcollections: [{ collection: Collection.Ingredients }],
  ...
  const ingredientsDB = useSelector((state: RootStateOrAny) => state.firestore.ordered.ingredients);

  const firestore = useFirestore();

  const deleteIngredient = (id: string): void => {
    if (dish) {
      firestore.collection(dish.collection).doc(dish.id).collection(Collection.Ingredients).doc(id).delete();
    }
  };
  ...
  useEffect(() => {
    ingredients.forEach((ingr: { id: string; title: string; portion: number }) =>
      ingredientsDB.filter((item: { id: string; title: string }) => item.title === ingr.title)
    );
  }, [ingredientsDB]);
